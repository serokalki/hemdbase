  ...
  return (
    <div className="card">
      ...
      <div className="grid g4">
        ...
        <Check k="nrbc" l="Nucleated red cells" /><Check k="giantPlt" l="Giant platelets" />
...
function Tally({ f, setF, applyDiff, wbc }){
  ...
  const bump = k => {
    ...
    setF({...f, tally:{...t, [k]:(t[k]||0)+1}, tallyLog:[...(f.tallyLog||[]), k]});
  };
  const undo = () => {
    const log = f.tallyLog || [];
    if(!log.length) return;
    const k = log[log.length-1];
    setF({...f, tally:{...t, [k]:Math.max(0,(t[k]||0)-1)}, tallyLog:log.slice(0,-1)});
  };
  const reset = () => setF({...f, tally:{}, tallyLog:[]});
  ...
  const apply = () => {
    ...
    applyDiff(out);
    setF({...f, tallyApplied:nowISO(), tallyTotal:total});
  };
    ...
    <>
      ...
      <div className="grid g4" style={{marginTop:12}}>
        <div><label>Cells to count</label>
          <select value={f.tallyTarget||100} onChange={e=>setF({...f, tallyTarget:e.target.value})}>
        ...
        <div><label>Nucleated red cells (/100 WBC)</label>
          <input inputMode="decimal" value={f.nrbc||""}
            onChange={e=>setF({...f, nrbc:e.target.value})} /></div>
        ...
        <div><label>Stain</label>
          <select value={f.stain||"Giemsa"} onChange={e=>setF({...f, stain:e.target.value})}>
        ...
        <div><label>Counted by</label>
          <input value={f.countedBy||""} onChange={e=>setF({...f, countedBy:e.target.value})} /></div>
        <div><label>Second reader</label>
          <input value={f.secondReader||""} onChange={e=>setF({...f, secondReader:e.target.value})}
            placeholder="required for abnormal films" /></div>
        <div style={{gridColumn:"span 2"}}><label>Smudge cells and counting notes</label>
          <input value={f.smudge||""} onChange={e=>setF({...f, smudge:e.target.value})}
